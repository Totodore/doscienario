...
export class EditTagsComponent {
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: [string, TabTypes], // [tabId, type]
  ...
  public updateTags(editedTags: string[]) {
    ...
    let docId = this.tabs.getTabFromId(this.data[0]).id;
  ...
  get el(): Blueprint | DocumentSock {
    if (this.data[1] === TabTypes.BLUEPRINT)
      ...
      return this.tabs.getTabFromId<DocumentComponent>(this.data[0]).doc;
  }
