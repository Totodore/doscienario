...
/******/ 	return __webpack_require__(__webpack_require__.s = "Ivsq");
/******/ })
/************************************************************************/
/******/ ({
...
/*!**********************************!*\
  ...
  \**********************************/
/*! exports provided: setImmediate, sortByRelevance */
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "sortByRelevance", function() { return sortByRelevance; });
function setImmediate(callback, ...args) {
    setTimeout(callback, 0, args);
}
function sortByRelevance(a, b, needle, accessor) {
    const aVal = accessor(a);
    const bVal = accessor(b);
    const commonAChar = aVal.split("").reduce((prev, curr) => prev + (needle.split(curr).length - 1), 0);
    const commonBChar = bVal.split("").reduce((prev, curr) => prev + (needle.split(curr).length - 1), 0);
...
/* harmony import */ var _utils_helpers__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./utils/helpers */ "GZ7p");
...
function computeDiff([oldContent, newContent]) {
    ...
    for (const change of diff__WEBPACK_IMPORTED_MODULE_0__["diffChars"](oldContent, newContent)) {
        if (change.added)
...
function search(needle, data) {
    ...
    const docsIds = docs.map(el => el.id).reduce((prev, curr) => [...prev, curr], []);
    const tagsIds = tags.map(el => el.name).reduce((prev, curr) => [...prev, curr], []);
    docs = docs
        .filter(doc => docsIds.includes(doc.id))
        .sort((a, b) => Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_1__["sortByRelevance"])(a, b, needle, (el) => el.title));
    tags = tags
        .filter(tag => tagsIds.includes(tag.name))
        .sort((a, b) => Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_1__["sortByRelevance"])(a, b, needle, el => el.name));
    /**
     * Alternative merge
     */
    let els;
